...
double Hp(double P[][3], int r, double A[], double Cp[], int *P_i)
{
   int		 i;
   double	 max_val, val;
...
double Hs(double P1[][3], int m1, double P2[][3], int m2, double A[], double Cs[], int *P1_i, int *P2_i)
{
...
int sub_dist_back(double P[][3], int stop_index, double *D_P, double Di_P[][4], int *Is, int c2)
{
   Boolean	  first, pass;
   int		  i, k, s, is, best_s;
...
int comp_sub_dist(double P[][3], int m, int *jo, int *Is, int *IsC, double near_pnt[], int near_indx[], double lambda[])
{
   Boolean	    pass, fail;
...
int sub_dist(double P[][3], int m, double near_pnt[], int near_indx[], double lambda[])
{
   int	      size;
...
void dist3d(double P1[][3], int m1, double P2[][3], int m2, double VP[], int near_indx[][2], double lambda[], int *m3)
{
   Boolean	    pass;
   int		    set_size, I[4], i, j, i_tab[4], j_tab[4], P1_i, P2_i, k;
   double	    Pk[4][3], Pk_subset[4][3], Vk[3], neg_Vk[3], Cp[3],
		    Gp;
...
Boolean get_new_plane(Couple couple)
{
...
Boolean Collision(Couple couple)
{
   Polyhedron	  polyhedron1, polyhedron2;
...
void init_polyhedron(Polyhedron polyhedron, double *verts, int m, double tx, double ty, double tz)
{
   int	      i;
   double     *p;
...
void move_polyhedron(Polyhedron polyhedron, double tx, double ty, double tz)
{
   polyhedron->trn[0]  += tx;  polyhedron->trn[1]  +=  ty;
   polyhedron->trn[2]  += tz;
...
/* Only test to Boost/c++ python*/
char const* greet(char* out)
{
   return out;
}
